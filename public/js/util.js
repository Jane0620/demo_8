 ...
 * @param {HTMLElement} container - 容器元素
 * @param {Array} students - 學生數據
 * @param {boolean} isManualMode - 是否為手動輸入模式
 * @param {Object} options - 配置選項
 * @param {string} options.measurementType - 測量類型（如 "height-weight" 或 "vision"）
 */
export function renderMeasureTable(container, students, isManualMode, options = {}) {
  ...
  const getRowHTML = (student, isRowReadOnly) => {
    const commonRowStart = `
      <tr>
        <td><input type="checkbox" data-student-id="${student.pid}" class="attendance-checkbox" ${student.attended ? 'checked' : ''} /></td>
        <td class="seat">${student.seat}</td>
        <td>${student.name}</td>
        <td>${student.sex == 1 ? '男' : '女'}</td>
    `;
    const commonRowEnd = `
        <td><input type="datetime-local" class="date-input" value="${formatDateTime(student.date)}" ${isRowReadOnly ? 'readonly' : ''} /></td>
      </tr>
    `;

    if (measurementType === 'height-weight') {
      return `${commonRowStart}
        <td><input type="number" class="height-input" value="${student.height || ''}" ${isRowReadOnly ? 'readonly' : ''} /></td>
        <td><input type="number" class="weight-input" value="${student.weight || ''}" ${isRowReadOnly ? 'readonly' : ''} /></td>
        ${commonRowEnd}
      `;
    } else if (measurementType === 'vision') {
      return `${commonRowStart}
        <td><input type="number" step="any" class="sight-input left-naked" value="${student.Sight0L || ''}" ${isRowReadOnly ? 'readonly' : ''} /></td>
        <td><input type="number" step="any" class="sight-input right-naked" value="${student.Sight0R || ''}" ${isRowReadOnly ? 'readonly' : ''} /></td>
        <td><input type="number" step="any" class="sight-input left" value="${student.SightL || ''}" ${isRowReadOnly ? 'readonly' : ''} /></td>
        <td><input type="number" step="any" class="sight-input right" value="${student.SightR || ''}" ${isRowReadOnly ? 'readonly' : ''} /></td>
        ${commonRowEnd}
      `;
    }
    return '';
      ...
      <table border="1" cellpadding="8" cellspacing="0" class="measure-table">
        <thead>
          <tr>
            ...
            <th>姓名</th>
            <th>性別</th>
            ${measurementType === 'height-weight' ? '<th>身高 (cm)</th><th>體重 (kg)</th>' : ''}
            ${measurementType === 'vision' ? '<th>左眼裸視</th><th>右眼裸視</th><th>左眼視力</th><th>右眼視力</th>' : ''}
            <th>檢測日期</th>
          </tr>
        </thead>
        <tbody>
          ${students.map(student => {
            const isRowReadOnly = !isManualMode || (isManualMode && !student.attended); // Modified logic
